...
```{r}
# Read county–ZIP crosswalk (Dec 2019) and ZIP-level demographics panel
county_zip_122019 <- as.data.table(readxl::read_excel("C:/OneDrive/data/county_zip_122019.xlsx"))
zip_demo <- readRDS("C:/OneDrive/data/nrs_branch_closure/zip_demographics_panel.rds")
setDT(zip_demo)
...
# (weighted sum: county gets sum(value * TOT_RATIO) over ZIPs in that county)
...
desc_stats <- reg_main[!is.na(period), .(
  N = .N,
  deposit_gr_mean = round(mean(growth_on_total_t1, na.rm = TRUE), 3),
  deposit_gr_sd   = round(sd(growth_on_total_t1, na.rm = TRUE), 3),
...
```{r}
p_density <- ggplot(reg_main, aes(x = growth_on_total_t1, fill = factor(any_closure_t))) +
  geom_density(alpha = 0.5) +
  labs(x = "Deposit Growth (t-1 to t+1, / total t-1)", y = "Density") +
  theme_minimal()

# ggsave("docs/figures/deposit_growth_density_by_closure.png", plot = p_density, width = 6, height = 4, dpi = 150)
p_density
...
g <- ggplot(coef_by_year, aes(x = year, y = coef)) +
  geom_point() +
  geom_line() +
...
# ggsave("docs/figures/closure_share_coef.png", plot = g, width = 6, height = 4, dpi = 150)
g
...
r_main <- list(
  "All"       = feols(..depvar ~ closure_share * sophisticated + closure_share * above_median_age + closure_share * above_median_income + ..controls | ..fixef, data = reg_main,                     vcov = ~ bank_id),
  "Pre 2012"  = feols(..depvar ~ closure_share * sophisticated + closure_share * above_median_age + closure_share * above_median_income + ..controls | ..fixef, data = reg_main[YEAR < 2012],       vcov = ~ bank_id),
  "2012-2019" = feols(..depvar ~ closure_share * sophisticated + closure_share * above_median_age + closure_share * above_median_income + ..controls | ..fixef, data = reg_main[YEAR %in% 2012:2019], vcov = ~ bank_id),
  "2020-2024" = feols(..depvar ~ closure_share * sophisticated + closure_share * above_median_age + closure_share * above_median_income + ..controls | ..fixef, data = reg_main[YEAR >= 2020],      vcov = ~ bank_id)
)

etable(r_main,order="closure_share")
...
g <- ggplot(es13_all, aes(x = event_time, y = estimate, group = period, color = period)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
...
ggsave("docs/figures/sunab_total_deposits.png", plot = g, width = 6, height = 4, dpi = 150)
g
```
...
g <- ggplot(es_all, aes(x = event_time, y = estimate, group = period, color = period)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  geom_line(linewidth = 0.8) +
  geom_point(aes(shape = period), size = 2.2) +
  geom_errorbar(
...
ggsave("docs/figures/sunab_deposits_of_survivors.png", plot = g, width = 6, height = 4, dpi = 150)
g
...
g <- ggiplot(sunab_model,
        main = "Sun & Abraham: Event Study (sampled never-treated controls)",
        xlab = "Years Relative to First Closure",
        ylab = "log(1 + remaining branches)")


ggsave("docs/figures/sunab_n_remaining_branches.png", plot = g, width = 6, height = 4, dpi = 150)
g
```
